...
def tgan (dataX, dataXs, parameters):
    ...
    Xs = tf.compat.v1.placeholder(tf.float32, [None, Max_Seq_Len, 2], name = "myinput_xs")
    Z = tf.compat.v1.placeholder(tf.float32, [None, Max_Seq_Len, z_dim], name = "myinput_z")
    Zs = tf.compat.v1.placeholder(tf.float32, [None, Max_Seq_Len, 2], name = "myinput_zs")
    ...
    def embedder (X, T):      
        ...
        with tf.compat.v1.variable_scope("embedder", reuse = tf.compat.v1.AUTO_REUSE):
            ...
            e_cell = tf.contrib.rnn.MultiRNNCell([rnn_cell(module_name) for _ in range(num_layers)])
    ...
    def embedderS (Xs, T):      
       ...
       with tf.compat.v1.variable_scope("embedders", reuse = tf.compat.v1.AUTO_REUSE):
            
            e_cell = tf.contrib.rnn.MultiRNNCell([rnn_cell(module_name) for _ in range(num_layers)])
                
            e_outputs, e_last_states = tf.nn.dynamic_rnn(e_cell, Xs, dtype=tf.float32, sequence_length = T)
    ...
    def generatorS (Zs, T):      
        ...
        with tf.compat.v1.variable_scope("generatorS", reuse = tf.compat.v1.AUTO_REUSE):
            
            e_cell = tf.nn.rnn_cell.MultiRNNCell([rnn_cell(module_name) for _ in range(num_layers)])
                
            e_outputs, e_last_states = tf.nn.dynamic_rnn(e_cell, Zs, dtype=tf.float32, sequence_length = T)
    ...
    # Generator
    E_hat = generator(Z, T)
    E_hat_s = generatorS(Zs,T)
    H_hat = supervisor(E_hat, T)
    H_hat_supervise = supervisor(H, T)
    H_hat_s = supervisor(E_hat_s, T)
    H_hat_supervise_s = supervisor(H_s, T)
        
    # Synthetic data
    X_hat = recovery(H_hat, T)
    X_hat_s = recovery(H_hat_s, T)
    
    # Discriminator
    H_hat_concat = tf.concat([H_hat, H_hat_s],axis=1)
    E_hat_concat = tf.concat([E_hat, E_hat_s],axis=1)
    H_concat = tf.concat([H, H_s],axis=0)
    Y_fake = discriminator(H_hat_concat, T)
    Y_real = discriminator(H_concat, T)     
    Y_fake_e = discriminator(E_hat_concat, T)
    #Y_fake_s = discriminator(H_hat_s, T)
   # Y_real_s = discriminator(H_s, T)     
    #Y_fake_e_s = discriminator(E_hat_s, T)
    ...
    G_loss_S_s = tf.losses.mean_squared_error(H_s[:,1:,:], H_hat_supervise_s[:,1:,:])
    ...
    G_loss_V1_s = tf.reduce_mean(np.abs(tf.sqrt(tf.nn.moments(X_hat_s,[0])[1] + 1e-6) - tf.sqrt(tf.nn.moments(X_s,[0])[1] + 1e-6)))
    G_loss_V2_s = tf.reduce_mean(np.abs((tf.nn.moments(X_hat_s,[0])[0]) - (tf.nn.moments(X_s,[0])[0])))
    
    G_loss_V = G_loss_V1 + G_loss_V2 + G_loss_V1_s + G_loss_V2_s
    
    # Summation
    G_loss = G_loss_U + gamma * G_loss_U_e + 100 *( tf.sqrt(G_loss_S) + tf.sqrt(G_loss_S_s)) + 100*G_loss_V 
            
    # Loss for the embedder network
    E_loss_T0 = tf.losses.mean_squared_error(X, X_tilde)
    E_loss_T0_s = tf.losses.mean_squared_error(X, X_s)
    
    #E_loss0 = 10*tf.sqrt(E_loss_T0)
    E_loss0 = 10*(tf.sqrt(E_loss_T0) + tf.sqrt(E_loss_T0_s))
    ...
    GS_solver = tf.train.AdamOptimizer().minimize(G_loss_S + G_loss_S_s, var_list = g_vars + s_vars)   
    ...
    for itt in range(iterations):
        ...
        Xs_mb = list(dataXs[i] for i in train_idx)
            
        # Train embedder        
        _, step_e_loss = sess.run([E0_solver, E_loss_T0], feed_dict={X: X_mb, T: T_mb, Xs: Xs_mb})
    ...
    for itt in range(iterations):
        ...
        Zs_mb = random_generator(batch_size, 2, T_mb, Max_Seq_Len)
        Z_mb = random_generator(batch_size, z_dim, T_mb, Max_Seq_Len)
        
        # Train generator       
        _, step_g_loss_s = sess.run([GS_solver, G_loss_S], feed_dict={Z: Z_mb, X: X_mb, T: T_mb, Xs: Xs_mb, Zs:Zs_mb})
    ...
    for itt in range(iterations):
        ...
        for kk in range(2):
            ...
            # Train generator
            _, step_g_loss_u, step_g_loss_s, step_g_loss_v = sess.run([G_solver, G_loss_U, G_loss_S, G_loss_V], feed_dict={Z: Z_mb, X: X_mb, T: T_mb, Xs: Xs_mb, Zs:Zs_mb})
            
            # Train embedder        
            _, step_e_loss_t0 = sess.run([E_solver, E_loss_T0], feed_dict={Z: Z_mb, X: X_mb, T: T_mb, Xs: Xs_mb, Zs:Zs_mb})   
        ...
        check_d_loss = sess.run(D_loss, feed_dict={X: X_mb, T: T_mb, Z: Z_mb, Xs: Xs_mb, Zs:Zs_mb})
        
        # Train discriminator
        
        if (check_d_loss > 0.15):        
            _, step_d_loss = sess.run([D_solver, D_loss], feed_dict={X: X_mb, T: T_mb, Z: Z_mb, Xs: Xs_mb, Zs:Zs_mb})
    ...
    X_hat_curr = sess.run(X_hat, feed_dict={Z: Z_mb, X: dataX, T: dataT, Xs: dataXs})    
