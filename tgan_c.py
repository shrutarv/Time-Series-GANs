...
def tgan (dataX, labels, parameters):
    ...
    c = tf.placeholder(tf.float32, [None, Max_Seq_Len, cond_dim], name = "condition")
    ...
    def generator (Z,c, T):      
        ...
        with tf.variable_scope("generator", reuse = tf.AUTO_REUSE):
            #repeated_encoding = tf.stack([c]*24, axis=1)
            inputs = tf.concat([Z, c], axis=2)

            
            e_cell = tf.nn.rnn_cell.MultiRNNCell([rnn_cell(module_name) for _ in range(num_layers)])
                
            e_outputs, e_last_states = tf.nn.dynamic_rnn(e_cell, inputs, dtype=tf.float32, sequence_length = T)
    ...
    def discriminator (H, c, T):
        ...
        with tf.variable_scope("discriminator", reuse = tf.AUTO_REUSE):
            #repeated_encoding = tf.stack([c]*24, axis=1)
            inputs = tf.concat([H, c], axis=2)

            d_cell = tf.nn.rnn_cell.MultiRNNCell([rnn_cell(module_name) for _ in range(num_layers)])
                
            d_outputs, d_last_states = tf.nn.dynamic_rnn(d_cell, inputs, dtype=tf.float32, sequence_length = T)
    ...
    for itt in range(iterations):
        ...
        X_mb = list(dataX[i] for i in train_idx)
        label_mb = list(labels[i] for i in train_idx)
        T_mb = list(dataT[i] for i in train_idx)        
        
        Z_mb = random_generator(batch_size, z_dim, T_mb, Max_Seq_Len)
        
        # Train generator       
        _, step_g_loss_s = sess.run([GS_solver, G_loss_S], feed_dict={Z: Z_mb, X: X_mb, T: T_mb, c:label_mb})
    ...
    for itt in range(iterations):
        ...
        for kk in range(2):
            ...
            # Train generator
            _, step_g_loss_u, step_g_loss_s, step_g_loss_v = sess.run([G_solver, G_loss_U, G_loss_S, G_loss_V], feed_dict={Z: Z_mb, X: X_mb, T: T_mb, c:label_mb})
        ...
        check_d_loss = sess.run(D_loss, feed_dict={X: X_mb, T: T_mb, Z: Z_mb, c:label_mb})
        
        # Train discriminator
        
        if (check_d_loss > 0.15):        
            _, step_d_loss = sess.run([D_solver, D_loss], feed_dict={X: X_mb, T: T_mb, Z: Z_mb, c:label_mb})
    ...
    X_hat_curr = sess.run(X_hat, feed_dict={Z: Z_mb, X: dataX, T: dataT, c:labels})    
